...
def reset_game():
    global dino, cacti, score, game_speed
    dino = Dino()
    cacti = []
    score = 0
    game_speed = 350
...
def draw_neural_network():
    for i in range(3):
        ...
        if networks[current_network].predict([0, 0, 0]):
            pyglet.text.Label("Jump", x=x + 5, y=350,
                              font_size=12, color=(0, 255, 0, 255), batch=batch).draw()
        else:
            pyglet.text.Label("Don't Jump", x=x + 5, y=350,
                              font_size=12, color=(255, 0, 0, 255), batch=batch).draw()
